...
// 	*xtset state year, yearly
	...
	*xtreg  OwnOrLeaseComputers  `controls' `var_interest' `CPI_decomp'  `year', vce(cluster state) fe allbase
	...
	esttab es_InternetAccess es_age es_prop_Female es_prop_dairy es_AdjCompCPI es_acres_per_oper es_IncomePerOperation, drop(`controls' _Istate_* year_*) stats (r2 N)	
	outreg2 using "Results\event_study_results_22", word excel replace
