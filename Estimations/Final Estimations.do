...
// Creating local lists for key variables 
	...
	// dropping 2023 due to extraneous values for imputed observations
	drop if year == 2023 // 42 obs deleted
	// comment out to look at models including 2023
//	
// ********************************************************************************
...
// 	outreg2 [OLS state_fixed year_fixed twfe] using "Results\twfe_regression_results_no23", word excel replace
********************************************************************************
...
// I want to decompose each of the variables of interest individually	
	...
	local year "year_1997	year_1998	year_1999	year_2000  year_2002	year_2003	year_2004	year_2005	year_2006	year_2007	year_2008	year_2009	year_2010	year_2011	year_2012	year_2013	year_2014	year_2015	year_2016	year_2017	year_2018	year_2019	year_2020	year_2021	year_2022"
...
// generating year variables 
	forval x = 1997/2022{
		gen year_`x' = 1 if year == `x'
		replace year_`x' = 0 if year != `x'
	}
	
	log using "event study", replace

// Internet Access
	xi: qui regress OwnOrLeaseComputers `internet_decomp' prop_dairy acres_per_oper prop_Female IncomePerOperation AdjCompCPI age `controls' i.state `year', vce(cluster state)
	estimates store event_study_InternetAccess

// CPI
	xi: qui regress OwnOrLeaseComputers `CPI_decomp' prop_dairy acres_per_oper prop_Female InternetAccess IncomePerOperation age `controls' i.state ib2001.year, vce(cluster state)
	estimates store event_study_AdjCompCPI

// Dairy
	xi: qui regress OwnOrLeaseComputers `prop_dairy_decomp' acres_per_oper prop_Female InternetAccess IncomePerOperation AdjCompCPI age `controls' i.state ib2001.year, vce(cluster state)
	estimates store event_study_prop_dairy

// Females
	xi: qui regress OwnOrLeaseComputers `gender_decomp' prop_dairy acres_per_oper InternetAccess IncomePerOperation AdjCompCPI age `controls' i.state ib2001.year, vce(cluster state)
	estimates store event_study_prop_Female	
	
// Age
	xi: qui regress OwnOrLeaseComputers `age_decomp' prop_dairy acres_per_oper prop_Female InternetAccess IncomePerOperation AdjCompCPI `controls' i.state ib2001.year, vce(cluster state)
	estimates store event_study_age
	
// Acres
	xi: qui regress OwnOrLeaseComputers `acres_decomp' prop_dairy prop_Female InternetAccess IncomePerOperation AdjCompCPI age `controls' i.state ib2001.year, vce(cluster state)
	estimates store event_study_acres_per_oper
	
//Income
	xi: qui regress OwnOrLeaseComputers `income_decomp' prop_dairy acres_per_oper prop_Female InternetAccess IncomePerOperation AdjCompCPI age `controls' i.state ib2001.year, vce(cluster state)
	estimates store event_study_IncomePerOper
	
	esttab event_study_IncomePerOper, keep (`income_decomp')
...
// Looping to generate coefficient charts for each of the key variables 
	...
	local var_interest "prop_dairy acres_per_oper prop_Female InternetAccess AdjCompCPI age IncomePerOperation"
	
	foreach x in `var_interest'{
		local y "`x'_*"
	
	coefplot event_study_`x', recast(connected)  keep(`y' year_1997  ) ///
