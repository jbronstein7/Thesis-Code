...
********************************************************************************
log using "twfe model 2022 unimputed", replace
// Starting with a basic OLS model, no fixed effects
	regress OwnOrLeaseComputers `var_interest' `controls', vce(cluster state)
	estimates store OLS
...
// The full twfe model 
	regress OwnOrLeaseComputers `var_interest' `controls' i.state ib2001.year, vce(cluster state)
	estimates store twfe

// exporting the results 
	esttab OLS state_fixed year_fixed twfe, keep(`var_interest') stats (r2 N)	
	outreg2 [OLS state_fixed year_fixed twfe] using "Results\twfe_results_22_noimp", word excel replace
...
// I want to decompose each of the variables of interest individually (with 2023)
	...
	local var_interest "prop_dairy InternetAccess acres_per_oper prop_Female IncomePerOperation AdjCompCPI age"
	...
	// Age 	
	xi: regress OwnOrLeaseComputers  `controls' prop_dairy InternetAccess acres_per_oper prop_Female IncomePerOperation AdjCompCPI `age_decomp' `year' i.state, vce(cluster state)
	...
	// Dairy
	xi: regress OwnOrLeaseComputers  `controls' InternetAccess acres_per_oper prop_Female IncomePerOperation AdjCompCPI age `prop_dairy_decomp'  `year' i.state, vce(cluster state)
	...
	// CPI
	xi: regress OwnOrLeaseComputers  `controls' prop_dairy InternetAccess acres_per_oper prop_Female IncomePerOperation age `CPI_decomp' `year' i.state, vce(cluster state)
	...
	// Acres
	xi: regress OwnOrLeaseComputers  `controls' `var_interest' `acres_decomp'  i.year i.state, vce(cluster state)
	...
	// Income
	xi: regress OwnOrLeaseComputers  `controls' prop_dairy InternetAccess acres_per_oper prop_Female AdjCompCPI age `income_decomp'  i.year i.state, vce(cluster state)
...
// Looping to generate coefficient charts for each of the key variables 
	local var_interest "InternetAccess age prop_Female prop_dairy"
	//age prop_Female prop_dairy AdjCompCPI acres_per_oper IncomePerOperation"	
...
// for CPI
local var_interest "AdjCompCPI"
foreach x in `var_interest'{
		local y "`x'_*"
	
	coefplot es_`x', recast(connected)  keep(`y' year_2007 ) ///
		ciopts(recast(rarea) color(emerald%25)) vertical  ///
		coeflabels(`x'_1997 = "97" `x'_1998 = "98" ///
		`x'_1999 = "99" `x'_2000 = "00" ///
		year_2007   = "01" ///
		...
		omitted baselevels	///
		order(`x'_1997	`x'_1998	`x'_1999	`x'_2000	year_2007  	`x'_2002	`x'_2003	`x'_2004	`x'_2005	`x'_2006	`x'_2007	`x'_2008	`x'_2009	`x'_2010	`x'_2011	`x'_2012	`x'_2013	`x'_2014	`x'_2015	`x'_2016	`x'_2017	`x'_2018	`x'_2019	`x'_2020	`x'_2021	`x'_2022	`x'_2023) title(Effect of `x' Across Time) ///
		xtitle(Event Year) ytitle(Marginal Effect)
	graph export "C:\Users\jbron\OneDrive - University of Arizona\Documents\School\Thesis\Raw Data\Charts\ES_`x'_22.png", as(png) name("Graph") replace	
